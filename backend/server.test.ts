...
async function createTestAgent(agentData: { email?: string; full_name?: string; phone_number?: string } = {}) {
  ...
  await pool.query(
    'UPDATE agents SET approved = true, approval_status = $1 WHERE agent_id = $2',
    ['approved', agent.agent_id]
  );
...
describe('Authentication - Agent Registration & Login', () => {
  ...
  it('should reject agent login when rejected', async () => {
    ...
    // Reject agent
    await pool.query(
      'UPDATE agents SET approval_status = $1, rejection_reason = $2 WHERE email = $3',
      ['rejected', 'Invalid license', email]
    );
...
describe('Authentication - Email Verification', () => {
  it('should verify email with valid token', async () => {
    ...
    const userResult = await pool.query(
      'SELECT email_verified, email_verification_token FROM users WHERE user_id = $1',
      [user.user_id]
    );
...
describe('Property Management - Read', () => {
  ...
  it('should show featured properties first', async () => {
    ...
    await pool.query(
      'UPDATE properties SET is_featured = true, featured_order = 1 WHERE property_id = $1',
      [featuredProperty.property_id]
    );
...
describe('Property Management - Update', () => {
  ...
  it('should create price history on price change', async () => {
    ...
    const historyResult = await pool.query(
      'SELECT * FROM property_price_history WHERE property_id = $1',
  ...
  it('should create status history on status change', async () => {
    ...
    // Verify status history created
    const historyResult = await pool.query(
      'SELECT * FROM property_status_history WHERE property_id = $1 AND new_status = $2',
...
describe('Property Management - Delete', () => {
  ...
  it('should cascade delete related photos', async () => {
    ...
    // Add photos
    await pool.query(
      'INSERT INTO property_photos (photo_id, property_id, image_url, display_order, is_primary, created_at) VALUES ($1, $2, $3, $4, $5, $6)',
      [generateUUID(), property.property_id, 'https://example.com/img1.jpg', 1, true, new Date().toISOString()]
    );
...
describe('Property Photos Management', () => {
  ...
  it('should delete photo', async () => {
    ...
    // Verify deleted
    const photosResult = await pool.query(
      'SELECT * FROM property_photos WHERE photo_id = $1',
      [photoId]
    );
    expect(photosResult.rows.length).toBe(0);
  });
...
describe('Inquiries - Creation and Management', () => {
  ...
  it('should increment property inquiry_count on inquiry creation', async () => {
    ...
    const propertyResult = await pool.query(
      'SELECT inquiry_count FROM properties WHERE property_id = $1',
      [property.property_id]
    );
  ...
  it('should filter agent inquiries by status', async () => {
    ...
    await pool.query(
      ...
      ['responded', inquiry1.body.inquiry_id]
    );
...
describe('Inquiries - Status and Reading', () => {
  it('should mark inquiry as read by agent', async () => {
    ...
    const inquiryResult = await pool.query(
      'SELECT agent_read, agent_read_at FROM inquiries WHERE inquiry_id = $1',
      [inquiryId]
    );
...
describe('Inquiries - Replies', () => {
  ...
  it('should update inquiry status to replied on agent reply', async () => {
    ...
    const inquiryResult = await pool.query(
      'SELECT status FROM inquiries WHERE inquiry_id = $1',
      [inquiryId]
    );
...
describe('Favorites Management', () => {
  ...
  it('should increment property favorite_count', async () => {
    ...
    const propertyResult = await pool.query(
      'SELECT favorite_count FROM properties WHERE property_id = $1',
      [property.property_id]
    );
...
describe('Saved Searches', () => {
  ...
  it('should delete saved search', async () => {
    ...
    // Verify deleted
    const result = await pool.query(
      'SELECT * FROM saved_searches WHERE saved_search_id = $1',
      [savedSearchId]
    );
    expect(result.rows.length).toBe(0);
  });
...
describe('Property Analytics', () => {
  it('should track property view', async () => {
    ...
    const viewsResult = await pool.query(
      'SELECT * FROM property_views WHERE property_id = $1',
      [property.property_id]
    );
  ...
  it('should increment property view_count', async () => {
    ...
    const propertyResult = await pool.query(
      'SELECT view_count FROM properties WHERE property_id = $1',
      [property.property_id]
    );
...
describe('Admin - Agent Approval', () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      'INSERT INTO admins (admin_id, email, password_hash, full_name, role, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)',
      [generateUUID(), adminEmail, 'admin123', 'Admin User', 'admin', new Date().toISOString(), new Date().toISOString()]
    );
...
describe('Open House RSVPs', () => {
  ...
  it('should increment open house rsvp_count', async () => {
    ...
    const openHouseResult = await pool.query(
      'SELECT rsvp_count FROM open_houses WHERE open_house_id = $1',
      [openHouseId]
    );
