...
app.use(morgan('dev'));
function createErrorResponse(message, error = null, errorCode = 'INTERNAL_ERROR') {
    const response = {
        success: false,
        message,
...
const authenticateToken = async (req, res, next) => {
    ...
    try {
        ...
        if (decoded.user_id) {
            const result = await pool.query('SELECT user_id, email, full_name, email_verified FROM users WHERE user_id = $1', [decoded.user_id]);
            ...
            const result = await pool.query('SELECT agent_id, email, full_name, approved, account_status FROM agents WHERE agent_id = $1', [decoded.agent_id]);
            if (result.rows.length === 0) {
                return res.status(401).json(createErrorResponse('Invalid token', null, 'INVALID_TOKEN'));
            }
            ...
            const result = await pool.query('SELECT admin_id, email, full_name, role FROM admins WHERE admin_id = $1', [decoded.admin_id]);
            if (result.rows.length === 0) {
                return res.status(401).json(createErrorResponse('Invalid token', null, 'INVALID_TOKEN'));
            }
...
app.post('/api/auth/register', async (req, res) => {
    try {
       ...
       created_at, updated_at) VALUES ($1, $2, true, true, false, true, false, $3, $4)`, [uuidv4(), user_id, now, now]);
        await pool.query(`INSERT INTO email_logs (log_id, recipient_email, recipient_type, recipient_id, email_type, 
       subject, template_name, status, created_at) VALUES ($1, $2, 'user', $3, 'email_verification', 
       'Verify Your Email Address', 'email_verification', 'pending', $4)`, [uuidv4(), validated.email, user_id, now]);
...
app.post('/api/auth/agent/register', async (req, res) => {
    try {
       ...
       RETURNING *`, [agent_id, validated.email.toLowerCase(), password_hash, validated.full_name, validated.phone_number,
        ...
        await pool.query(`INSERT INTO agent_notification_preferences (preference_id, agent_id, new_inquiry_received, 
       ...
       VALUES ($1, $2, true, true, true, true, false, 'instant', false, $3, $4)`, [uuidv4(), agent_id, now, now]);
        await pool.query(`INSERT INTO email_logs (log_id, recipient_email, recipient_type, recipient_id, email_type, 
       subject, template_name, status, created_at) VALUES ($1, $2, 'agent', $3, 'email_verification', 
       'Verify Your Email Address', 'agent_email_verification', 'pending', $4)`, [uuidv4(), validated.email, agent_id, now]);
...
app.post('/api/auth/verify-email', async (req, res) => {
    try {
        ...
        const now = new Date().toISOString();
        const userResult = await pool.query('UPDATE users SET email_verified = true, email_verification_token = NULL, updated_at = $1 WHERE email_verification_token = $2 AND email_verified = false RETURNING *', [now, validated.token]);
        if (userResult.rows.length > 0) {
            return res.json({ success: true, message: 'Email verified successfully' });
        }
        const agentResult = await pool.query('UPDATE agents SET email_verified = true, email_verification_token = NULL, updated_at = $1 WHERE email_verification_token = $2 AND email_verified = false RETURNING *', [now, validated.token]);
...
app.post('/api/auth/forgot-password', async (req, res) => {
    try {
        ...
        const now = new Date().toISOString();
        let found = false;
        const userResult = await pool.query('UPDATE users SET password_reset_token = $1, password_reset_expires = $2, updated_at = $3 WHERE email = $4 RETURNING user_id, email', [reset_token, expires_at, now, email.toLowerCase()]);
        if (userResult.rows.length > 0) {
            found = true;
            await pool.query(`INSERT INTO email_logs (log_id, recipient_email, recipient_type, recipient_id, email_type, subject, status, created_at) 
         VALUES ($1, $2, 'user', $3, 'password_reset', 'Reset Your Password', 'pending', $4)`, [uuidv4(), email, userResult.rows[0].user_id, now]);
        }
        if (!found) {
            const agentResult = await pool.query('UPDATE agents SET password_reset_token = $1, password_reset_expires = $2, updated_at = $3 WHERE email = $4 RETURNING agent_id, email', [reset_token, expires_at, now, email.toLowerCase()]);
            if (agentResult.rows.length > 0) {
                found = true;
                await pool.query(`INSERT INTO email_logs (log_id, recipient_email, recipient_type, recipient_id, email_type, subject, status, created_at) 
           VALUES ($1, $2, 'agent', $3, 'password_reset', 'Reset Your Password', 'pending', $4)`, [uuidv4(), email, agentResult.rows[0].agent_id, now]);
            }
        }
...
app.get('/api/users/me', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT user_id, email, full_name, phone_number, email_verified, profile_photo_url, location, created_at, updated_at FROM users WHERE user_id = $1', [req.user.user_id]);
        if (result.rows.length === 0) {
...
app.put('/api/users/me', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE users SET ${updates.join(', ')} WHERE user_id = $${paramCount} RETURNING user_id, email, full_name, phone_number, email_verified, profile_photo_url, location, created_at, updated_at`, values);
        res.json(result.rows[0]);
    }
    catch (error) {
        res.status(500).json(createErrorResponse('Internal server error', error, 'INTERNAL_SERVER_ERROR'));
...
app.get('/api/properties', async (req, res) => {
    try {
       ...
       ${whereClause}
       ORDER BY ${orderBy}
       LIMIT $${paramCount} OFFSET $${paramCount + 1}`, [...values, validated.limit, validated.offset]);
        res.json({
            data: result.rows,
            pagination: {
...
app.post('/api/properties', authenticateToken, async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO property_status_history (history_id, property_id, old_status, new_status, changed_by_agent_id, notes, changed_at)
       VALUES ($1, $2, 'draft', $3, $4, 'Property created', $5)`, [uuidv4(), property_id, validated.status, req.agent.agent_id, now]);
...
app.get('/api/properties/:property_id', async (req, res) => {
    try {
        const result = await pool.query(`SELECT p.*, a.full_name as agent_name, a.agency_name, a.phone_number as agent_phone, a.email as agent_email
       FROM properties p
       LEFT JOIN agents a ON p.agent_id = a.agent_id
       WHERE p.property_id = $1`, [req.params.property_id]);
        if (result.rows.length === 0) {
...
app.post('/api/inquiries', async (req, res) => {
    try {
        ...
        const now = new Date().toISOString();
        const result = await pool.query(`INSERT INTO inquiries (
        ...
        await pool.query('UPDATE properties SET inquiry_count = inquiry_count + 1 WHERE property_id = $1', [validated.property_id]);
        await pool.query(`INSERT INTO email_logs (log_id, recipient_email, recipient_type, recipient_id, email_type, subject, status, created_at)
       VALUES ($1, (SELECT email FROM agents WHERE agent_id = $2), 'agent', $2, 'new_inquiry', 'New Inquiry Received', 'pending', $3)`, [uuidv4(), validated.agent_id, now]);
        res.status(201).json(result.rows[0]);
    }
    catch (error) {
        res.status(500).json(createErrorResponse('Internal server error', error, 'INTERNAL_SERVER_ERROR'));
...
app.post('/api/favorites', authenticateToken, async (req, res) => {
    try {
        ...
        const now = new Date().toISOString();
        const existing = await pool.query('SELECT favorite_id FROM favorites WHERE user_id = $1 AND property_id = $2', [req.user.user_id, validated.property_id]);
        if (existing.rows.length > 0) {
            return res.status(409).json(createErrorResponse('Property already in favorites', null, 'ALREADY_EXISTS'));
        }
        const result = await pool.query('INSERT INTO favorites (favorite_id, user_id, property_id, created_at) VALUES ($1, $2, $3, $4) RETURNING *', [favorite_id, req.user.user_id, validated.property_id, now]);
        await pool.query('UPDATE properties SET favorite_count = favorite_count + 1 WHERE property_id = $1', [validated.property_id]);
        res.status(201).json(result.rows[0]);
    }
    catch (error) {
        res.status(500).json(createErrorResponse('Internal server error', error, 'INTERNAL_SERVER_ERROR'));
